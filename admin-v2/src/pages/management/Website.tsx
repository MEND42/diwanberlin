...
const SECTIONS: SectionDef[] = [
  ...
  },
  {
    ...
    blocks: [
      { key: 'about_de', label: 'Text (DE)', placeholder: 'Das Diwan Berlin ist…', multiline: true },
      { key: 'about_fa', label: 'Text (FA)', placeholder: 'دیوان برلین…',           multiline: true },
    ],
  },
];
          ...
          <div key={b.key}>
            ...
            {b.multiline ? (
              <textarea
                ...
                dir={b.key.endsWith('_fa') ? 'rtl' : undefined}
              />
            ) : (
              <input
                value={blocks[b.key] ?? ''}
...
export function Website() {
  ...
  useEffect(() => {
    ...
    (rawBlocks as SiteBlock[]).forEach(b => { map[b.key] = b.value; });
    setLocalValues(map);
  }, [rawBlocks]);
  ...
  async function saveSection(section: SectionDef) {
    ...
    try {
      for (const b of section.blocks) {
        ...
        const value    = localValues[b.key] ?? '';
        if (existing) {
          await siteApi.update(existing.id, { key: b.key, value });
        } else if (value) {
          await siteApi.create({ key: b.key, value });
        }
      }
